...
dixnames=($(grep -m 1 "^PairModeDixNames=" testvoc.conf | cut -d = -f 2))
...
for i in "${!modes[@]}"; do
    ...
    if [[ $ENCLITICS = true ]] && [[ $TRIMMED = false ]]; then
        bash generation.sh -e "<prn><enc>" -u -a ${modes[$i]} ${dixnames[$i]} > .testvoc
    elif [[ $ENCLITICS = true ]]; then
        bash generation.sh -e "<prn><enc>" -a ${modes[$i]} ${dixnames[$i]} > .testvoc
    elif [[ $TRIMMED = false ]]; then
        bash generation.sh -u -a ${modes[$i]} ${dixnames[$i]} > .testvoc
    else
        bash generation.sh -a ${modes[$i]} ${dixnames[$i]} > .testvoc
    fi
